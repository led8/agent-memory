...
[WARNING]
====
This integration targets Microsoft Agent Framework v1.0.0b251223 (preview). APIs may change before GA release (expected Q1 2026). See <<api-compatibility>> for version requirements and migration guidance.
====
...
async with MemoryClient(settings) as client:
    ...
    await store.add_messages([
        ChatMessage(role=MessageRole.USER, content="I love Nike running shoes"),
        ChatMessage(role=MessageRole.ASSISTANT, content="Nike makes great running shoes!"),
    ])
...
Create function tools for agent memory operations:
...
tools = create_memory_tools(
    ...
    include_knowledge=True,    # search_knowledge, remember_fact
    include_traces=True,       # find_similar_tasks
...
result = await execute_memory_tool(
    ...
    arguments={"category": "brand", "preference": "Nike"},
    memory_client=client,
    session_id="user-123",
)
----
...
await record_agent_trace(
    ...
    steps=[
        {"tool": "search_products", "input": {"query": "running shoes"}, "output": "..."},
        {"tool": "recall_preferences", "input": {"category": "brand"}, "output": "Nike"},
    ],
    outcome="success",
    result="Recommended Nike Pegasus 40 ($130)",
)
...
The `invoking()` hook composes context from all memory types:
...
Here's a complete shopping assistant example:
...
| `agent_framework.ChatMessageStoreProtocol`

| `ChatMessage`
| `agent_framework.ChatMessage`

| `MessageRole`
| `agent_framework.MessageRole`
...
| `invoking(context)`
| Called before agent invocation; injects memory context

| `invoked(context, response)`
| Called after agent response; extracts and stores memories

| `serialize()`
| Serialize provider state

| `deserialize(state, client)`
...
| `create_memory_tools(...)`
| Create agent function tools

| `execute_memory_tool(name, args, client, session)`
