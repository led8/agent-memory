...
|`model`
|str
|`text-embedding-3-small` or `BAAI/bge-small-en-v1.5`
|Model name. Defaults to OpenAI small for `OPENAI` and BGE small for `SENTENCE_TRANSFORMERS`
...
|int
|1536 or 384
|Embedding dimensions. Defaults follow the provider/model when not explicitly set
...
|`BAAI/bge-small-en-v1.5` (default), `all-MiniLM-L6-v2`, `all-mpnet-base-v2`, etc.
|Runs locally. `BAAI/bge-small-en-v1.5` is the recommended V1 default
|===
