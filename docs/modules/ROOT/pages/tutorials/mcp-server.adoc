...
TIP: If you installed with `uvx`, use `"command": "uvx"` and `"args": ["neo4j-agent-memory[mcp]", "mcp", "serve", "--password", "<your-local-password>"]` instead.
...
settings = MemorySettings(
    neo4j={"uri": "bolt://localhost:7687", "password": "<your-local-password>"}
)
