...
* Completed xref:tutorials/first-agent-memory.adoc[Build Your First Memory-Enabled Agent] tutorial
* OpenAI API key
...
async def get_or_create_user(name: str) -> str:
    ...
    # Create new user
    user, _ = await memory_client.long_term.add_entity(
        name=name,
...
async def build_context(user_id: str, session_id: str, query: str) -> str:
    ...
    # Get recent messages
    conversation = await memory_client.short_term.get_conversation(session_id=session_id)
    recent_messages = conversation.messages[-5:] if conversation.messages else []

    # Get preferences
    prefs = await memory_client.long_term.search_preferences(query=query)

    # Get relevant entities
    entities = await memory_client.long_term.search_entities(query=query, limit=10)
...
async def chat(
    ...
    for entity in extraction.entities:
        _, _ = await memory_client.long_term.add_entity(
            name=entity.name,
...
async def show_memory_stats(user_id: str):
    ...
    # Get preferences
    prefs = await memory_client.long_term.search_preferences(query="")
    if prefs:
        print("\n🏷️ Preferences:")
        for pref in prefs:
...
async def chat(
    ...
    try:
        ...
        step1 = await memory_client.reasoning.add_step(
            trace_id=trace.id,
            thought="Extracting entities from user message",
        )
        ...
        step2 = await memory_client.reasoning.add_step(
            trace_id=trace.id,
            thought="Retrieving personalized context",
        )
        ...
        await memory_client.reasoning.complete_trace(
            trace_id=trace.id,
            outcome=f"Generated response ({len(assistant_message)} chars)",
            success=True,
        )
...
* xref:tutorials/knowledge-graph.adoc[Build a Knowledge Graph from Documents] - Extract knowledge from documents
