...
async def main():
    ...
    async with MemoryClient(settings) as client:
        ...
        stored_prefs = await client.long_term.search_preferences(query="")
        ...
        prefs = await client.long_term.search_preferences(query="recommend running shoes")
        # Search for relevant entities
        entities = await client.long_term.search_entities(query="recommend running shoes", limit=5)

        print(f"\nPreferences found: {len(prefs)}")
...
* xref:tutorials/conversation-memory.adoc[Add Conversation Memory to a Chatbot] - Build a chatbot that remembers across sessions
* xref:tutorials/knowledge-graph.adoc[Build a Knowledge Graph from Documents] - Extract and connect knowledge from documents
